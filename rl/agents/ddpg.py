...
class DDPGAgent(Agent):
    ...
    def update_sim_forward_models_hard(self):
    ...
    def select_action(self, state):
        ...
        action = self.sim_forward_actor.predict_on_batch(batch).flatten()
    ...
    def backward(self, reward, terminal=False):
        ...
        with self.sess.graph.as_default():
            while self.back_step< self.nb_steps: 
                mutex.acquire()
                training_count+=1
                ...
                if can_train_either and self.step % self.train_interval == 0:
                    ...
                    if self.step > self.nb_steps_warmup_critic:
                        if (self.step-self.nb_steps_warmup_critic) == 1:
                            self.backward_start_flag = True
                            self.step+=1
